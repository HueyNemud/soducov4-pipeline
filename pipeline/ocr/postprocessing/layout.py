...
from typing import Any, Dict, Iterable, Sequence, Tuple, Literal, Annotated
import numpy as np
...
def box_affinity(
    ...
    dist_y = np.maximum(0, l_y1[:, None] - anchors_y) + np.maximum(0, anchors_y - l_y2[:, None])
    dist_x = np.maximum(0, l_x1[:, None] - anchors_x) + np.maximum(0, anchors_x - l_x2[:, None])
...
def compute_local_reading_order(
    ...
    **Drawbacks & Constraints**
    ...
    for layout_id in range(num_layouts):
        ...
        if direction == ReadingDirection.LTR:
            starts, ends = x_min[line_indices], (x_min[line_indices] + x_max[line_indices]) / 2
        else:
            starts, ends = (x_min[line_indices] + x_max[line_indices]) / 2, x_max[line_indices]
        
        # Map coordinates to bin indices and fill density histogram
        density = np.zeros(n_bins, dtype=np.int32)
        start_bins = np.clip(((starts - block_x_min) / bin_size).astype(int), 0, n_bins - 1)
        ...
        threshold = np.percentile(density[density > 0], 10) if np.any(density > 0) else 0
        ...
        if col_starts.size == 0:
            ...
            column_spans = np.column_stack([
                block_x_min + col_starts * bin_size, 
                block_x_min + col_ends * bin_size
            ])
        ...
        dist_to_columns = np.maximum(0, np.maximum(column_spans[:, 0] - anchors, anchors - column_spans[:, 1]))
        ...
        column_priority = column_assignments if direction == ReadingDirection.LTR else -column_assignments
        sort_indices = np.lexsort((y_centers[line_indices], column_priority))

        # Map back to global line indices (1-based ordering)
        reading_order[layout_id, line_indices[sort_indices]] = np.arange(1, line_indices.size + 1)
...
class SuryaLayoutPostProcessor:
    ...
    class Parameters:
        ...
        decay_factor: float = Field(2.0, ge=0.1, description="Controls how fast the affinity drops as the line moves away from the layout box.") 
        anchor_offset: float = Field(0.20, ge=0, le=1.0, description="Leading edge position (0.0 = edge, 0.5 = center). Default is 20% from edge.")
        
        # Column detection parameters
        n_bins: int = Field(400, gt=0, description="Number of bins for horizontal projection histogram. Higher values increase resolution but may over-segment.")
        occupancy_percentile: float = Field(10.0, ge=0, le=100, description="Percentile threshold to determine occupied bins in column detection.")
        
        # Filtering
        confidence_threshold: float = Field(0.8, ge=0, le=1.0, description="Minimum confidence for text lines to be considered reliable. Text lines below this threshold are ignored.")
        
        # Debugging
        debug_plots: bool = Field(False, description="Enable generation of debug plots for visual inspection. Warning: will dramatically increase processing time.")
        debug_dir: str = Field("./debug_plots", description="Directory to save debug plots.")


    def process_ocr_output(self, document: OCRDocument, params: Parameters) -> OCRDocument:
        """
        ...
        for page_idx, (layout, ocr) in enumerate(zip(document.layout, document.ocr)):
            logger.debug(f"Processing Page {page_idx}: {len(layout.bboxes)} blocks, {len(ocr.text_lines)} lines.")
            
            # 1. Prepare coordinate matrices
            layout_boxes = np.array([b.bbox for b in layout.bboxes], dtype=np.int32)
            all_text_boxes = np.array([l.bbox for l in ocr.text_lines], dtype=np.int32) 
            
            # 2. Filter lines by confidence
            
            is_reliable = np.array([l.confidence if l.confidence is not None else 1.0 >= params.confidence_threshold for l in ocr.text_lines], dtype=bool)
            
            if not np.any(is_reliable) or layout_boxes.size == 0:
                logger.warning(f"Skipping Page {page_idx}: No valid content after filtering.")
            ...
            reliable_objects = [line for i, line in enumerate(ocr.text_lines) if is_reliable[i]]            
            
            # 3. Spatial Assignment
            # Compute affinity and assign each line to the highest scoring layout block
            affinity_matrix = box_affinity(layout_boxes, reliable_boxes,
            ...
            assignment_mask = np.zeros((layout_boxes.shape[0], reliable_boxes.shape[0]), dtype=bool)
            ...
            reading_indices = compute_local_reading_order(assignment_mask, reliable_boxes, 
                                                            direction=params.reading_direction,
        ...
        if params.debug_plots:
            self._export_debug(processed_pages, debug_scores, output_dir=params.debug_dir)
    ...
    # Debug helpers
    # ------------------------------------------------------------------
    def _export_debug(self, ocr_doc: OCRDocument, line_probs_dict: dict, output_dir: str):
        ...
        def get_style(line, cmap):
            ...
            if line.layout_ix is None or (line.reading_order_ix or -1) < 0:
                return {"facecolor": "black", "edgecolor": "red", "alpha": 1.0, "hatch": "////", "text_color": "red"}
            ...
            return {"facecolor": color, "edgecolor": color, "alpha": alpha, "text_color": "white"}

        for p_idx, (layout, ocr) in enumerate(zip(ocr_doc.layout, ocr_doc.ocr)):
            bbox = layout.image_bbox
            configs = [
                ("layout", lambda ax, l=layout, o=ocr: [draw_poly(ax, b.polygon, text=str(i), facecolor=cmaps["layout"](i % cmaps["layout"].N), edgecolor="black", alpha=0.3) for i, b in enumerate(l.bboxes)]),
                ("assignment", lambda ax, l=layout, o=ocr: [draw_poly(ax, line.polygon, text=str(line.layout_ix or "?"), **get_style(line, cmaps["layout"])) for line in o.text_lines]),
                ("reading_order", lambda ax, l=layout, o=ocr: _draw_reading_flow(ax, o, cmaps["order"], draw_poly))
            ]
            ...
            for suffix, draw_func in configs:
                ...
                ax.set_title(f"Page {p_idx} - {suffix.replace('_', ' ').title()}", fontsize=20)
                
                fig.savefig(os.path.join(output_dir, f"page_{p_idx}_{suffix}.png"), bbox_inches="tight")
...
def _draw_reading_flow(ax, ocr, cmap, draw_func):
    ...
    orders = [l.reading_order_ix for l in ocr.text_lines if (l.reading_order_ix or 0) > 0]
    max_o = max(orders) if orders else 1
    for l in ocr.text_lines:
        if (l.reading_order_ix or -1) < 0:
            draw_func(ax, l.polygon, facecolor="black", edgecolor="black", alpha=0.9, hatch="XXX", text="!", text_color="red")
        else:
            draw_func(ax, l.polygon, facecolor=cmap(l.reading_order_ix / max_o), edgecolor="black", alpha=0.8, text=str(l.reading_order_ix), text_color="white")