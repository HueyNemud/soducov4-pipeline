...
class OCR(PipelineStage[OCRDocument, OCRParameters]):
    ...
    def _log_pages(self, document: OCRDocument) -> None:
        ...
        for p_idx, (p_res, l_res) in enumerate(zip(document.ocr, document.layout)):
            ...
            self.logger.info(f"📄 PAGE {p_idx + 1}\n{tabulate.tabulate(rows, tablefmt='github')}")

    def _save_page_debug_visuals(self, ctx: RunContext, document: OCRDocument, pdf_path: Path) -> None:
        """
        ...
        with pdfium.PdfDocument(str(pdf_path)) as pdf:
            for idx, page_result in enumerate(document.ocr):
                ...
                for line in page_result.text_lines:
                    ...
                    color = colors.setdefault(line.layout_ix, (random.random(), random.random(), random.random()))
                ...
                ax.axis('off')
                fig.savefig(debug_dir / f"page_{idx + 1:03d}.png", bbox_inches="tight", pad_inches=0)
                plt.close(fig)